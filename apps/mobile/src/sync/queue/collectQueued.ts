...
/**
 * Extract the last-known module payloads from a queue, tolerating
 * corrupted rows (non-objects, wrong types, unknown module names,
 * missing data). Later entries overwrite earlier ones for the same
 * module since the queue is append-ordered. 1:1 port of the web
 * implementation at `apps/web/src/core/cloudSync/queue/collectQueued.ts`
 * so replay semantics match across platforms.
 */
