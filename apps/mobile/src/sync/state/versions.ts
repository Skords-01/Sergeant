/**
 * Per-user, per-module server version numbers (optimistic-concurrency
 * token). Stored under one MMKV key as a nested map keyed by userId so
 * signing in as a different account on the same device does not clobber
 * cached versions. Mirror of
 * `apps/web/src/core/cloudSync/state/versions.ts`.
 */
