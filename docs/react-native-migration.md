...
- `apps/mobile` — Jest (`jest-expo`) + React Native Testing Library для
  unit/component. E2E — **Detox** (обрано у Q8), конфіг у
  `apps/mobile/.detoxrc.js`, перший сьют — `apps/mobile/e2e/finyk-manual-expense.e2e.ts`
  (Фінік → Overview → Transactions → manual expense → асерт у списку).
  Детальніше — у `apps/mobile/e2e/README.md`.
- Dev-only E2E auth-bypass: `EXPO_PUBLIC_E2E=1` вимикає Better Auth
  redirect у `app/(tabs)/_layout.tsx` і активує `@config-plugins/detox`
  у `app.config.ts`; у production IPA/AAB змінна не присутня, тож вона
  компілюється геть Metro-м при релізних білдах.
...
- Preview-builds через **EAS Update** для internal testing без
  повного rebuild.
- **Detox iOS** — `.github/workflows/detox-ios.yml` (runner `macos-14`,
  `pnpm install --frozen-lockfile` → `expo prebuild --platform ios` →
  `pod install` → `pnpm e2e:build:ios` → `pnpm e2e:test:ios`). Тригериться
  лише на mobile-scoped шляхах (`apps/mobile/**`, relevant `packages/**`,
  `pnpm-lock.yaml`, сам workflow) щоб не палити macOS-хвилини на
  web-only PR-ах. Android CI — TODO (див. follow-up до PR Detox-infra).
...
- **Q8. E2E тестування.** ✅ **Detox.**
  ...
  (щоб не писати тести пост-фактум). Infra приземлено: `.detoxrc.js`
  з iOS sim + Android emu конфігами, `apps/mobile/e2e/` (jest config,
  `environment.js`, `setup.ts`, `helpers.ts`, `README.md`), перший сьют
  `finyk-manual-expense.e2e.ts` (Overview → Transactions → ManualExpenseSheet
  → асерт нового `finyk-tx-row-*`), dev-only auth-bypass через
  `EXPO_PUBLIC_E2E=1` у `app/(tabs)/_layout.tsx`, `@config-plugins/detox`
  підключений умовно в `app.config.ts`, iOS CI-лейн у
  `.github/workflows/detox-ios.yml`. Android-лейн — follow-up.
