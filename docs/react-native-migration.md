...
- `apps/mobile/src/components/ui/Banner.tsx` — `Banner`
  (PR `<PR-BANNER>`). Дзеркалить `BannerVariant` (`info` / `success` /
  `warning` / `danger`), rounded-2xl border + bg токени.
...
| 1   | Спільна UI-основа для RN (+ NativeWind + MMKV + Dev Client) | 🔵 In progress | 0                            | NativeWind (Q5) ✅ #403, MMKV-сховище (Q3) ✅ #404, `@sergeant/design-tokens` (R6) ✅ #406, Expo Dev Client через EAS (Q4) ✅ #408/#410, `Button` ✅ #407, `Card` ✅ #413 (+ jest-expo setup), `Input` ✅ #417, `Banner` ✅ `<PR-BANNER>`. Залишилось: `Toast`, `Skeleton`, `ConfirmDialog`, `Sheet` — кожен окремим малим PR-ом по шаблону #407. |
