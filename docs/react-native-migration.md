...
- `apps/mobile/src/components/ui/ConfirmDialog.tsx` — `ConfirmDialog`
  (PR `<PR-CONFIRM-DIALOG>`). Bottom-sheet-style підтвердження на
...
- `apps/mobile/src/components/ui/Sheet.tsx` — `Sheet` bottom-sheet
  shell (PR `<PR-SHEET>`). Обгортка навколо RN-вбудованого `Modal`
...
| 1   | Спільна UI-основа для RN (+ NativeWind + MMKV + Dev Client) | 🔵 In progress | 0                            | NativeWind (Q5) ✅ #403, MMKV-сховище (Q3) ✅ #404, `@sergeant/design-tokens` (R6) ✅ #406, Expo Dev Client через EAS (Q4) ✅ #408/#410, `Button` ✅ #407, `Card` ✅ #413 (+ jest-expo setup), `Input` ✅ #417, `Banner` ✅ #419, `Toast` ✅ #421, `Skeleton` ✅ #423, `ConfirmDialog` ✅ `<PR-CONFIRM-DIALOG>`. Залишилось: `Sheet` — окремим малим PR-ом по шаблону #407. |
