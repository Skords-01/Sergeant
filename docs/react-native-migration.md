...
- **Комора (mobile, зроблено):** `useNutritionPantries` + `pages/Pantry.tsx`,
  ...
  `parseLoosePantryText` / `groupItemsByCategory`, кілька складів. Без
  `nutritionApi.parsePantry` (серверний AI-розбір списку) — ще ні.
- Фото-їжа: `expo-image-picker` (галерея) + `expo-camera`
  (shot на льоту) → `ImageManipulator` для resize → send to server (не
  портовано).
