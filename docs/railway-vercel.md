...
| `ALLOWED_ORIGINS`                | URL фронту на Vercel, напр. `https://твій-проєкт.vercel.app` (через кому, якщо кілька). **Важливо:** додай і URL Vercel preview deployments, інакше Better Auth поверне `403 Invalid origin` при логіні з прев'ю. Приклад: `https://sergeant.vercel.app,https://sergeant-git-*-skords-01s-projects.vercel.app` (wildcard не підтримується — додавай конкретні URL). |
...
| `VAPID_EMAIL`                    | **Обов'язково для push-нотифікацій.** Контактний email у форматі `mailto:you@example.com`. Push-сервіси (FCM, Mozilla) використовують його для зв'язку при проблемах. Без цієї змінної `/api/push/vapid-public` повертає 503 і push не працює.                                                                                                                      |
| `VAPID_PUBLIC_KEY`               | Публічний VAPID-ключ. Згенеруй пару: `npx web-push generate-vapid-keys`.                                                                                                                                                                                                                                                                                            |
| `VAPID_PRIVATE_KEY`              | Приватний VAPID-ключ (з тієї ж команди вище).                                                                                                                                                                                                                                                                                                                       |
