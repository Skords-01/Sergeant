...
import { makeDefaultWarmup, makeDefaultCooldown } from "../../hooks/useWorkouts.js";
...
function WarmupCooldownChecklist({ title, items, onToggle, onInit, color }) {
  if (!items) {
    return (
      <div className={`rounded-xl border border-dashed ${color.border} px-3 py-2.5 flex items-center justify-between gap-2`}>
  ...
  return (
    <details className={`rounded-xl border ${color.border} bg-panelHi/50 px-3 py-2`} open={doneCount < total}>
      <summary className={`text-xs font-semibold ${color.text} cursor-pointer select-none flex items-center justify-between`}>
        <span>{title}</span>
        <span className={`ml-2 text-[10px] font-bold tabular-nums ${doneCount === total ? "text-success" : color.text}`}>
          {doneCount}/{total}
        </span>
      </summary>
        ...
        {(items || []).map((item) => (
          <li key={item.id} className="flex items-center gap-2">
            <button
              ...
              aria-label={item.done ? "Позначити як незавершене" : "Позначити як завершене"}
            >
              {item.done && (
            ...
            <span className={`text-xs ${item.done ? "line-through text-subtle" : "text-text"}`}>{item.label}</span>
          </li>
        ))}
      </ul>
    </details>
...
function SupersetBadge({ type }) {
  return (
    <span className={`text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded-full ${type === "circuit" ? "bg-accent/15 text-accent border border-accent/30" : "bg-success/15 text-success border border-success/30"}`}>
...
export function ActiveWorkoutPanel({
  ...
  const handleCreateSuperset = (type) => {
    ...
    const newGroups = [...groups.filter((g) => !g.itemIds.some((id) => groupSelected.has(id))), newGroup];
  ...
  const renderItem = (it) => {
    ...
    const cardioMetrics = it.type === "distance" ? calcCardioMetrics(it.distanceM, it.durationSec) : null;
            ...
            {cardioMetrics && (
              <div className="grid grid-cols-2 gap-2">
                <div className="rounded-xl border border-line bg-bg px-3 py-2 text-center">
                  <div className="text-[9px] font-bold text-subtle uppercase tracking-widest">Темп</div>
                  <div className="text-sm font-bold text-text tabular-nums">{cardioMetrics.pace}</div>
                </div>
                <div className="rounded-xl border border-line bg-bg px-3 py-2 text-center">
                  <div className="text-[9px] font-bold text-subtle uppercase tracking-widest">Швидкість</div>
                  <div className="text-sm font-bold text-text tabular-nums">{cardioMetrics.speed}</div>
                </div>
              </div>
            )}
          </div>
  ...
  const renderItemsList = () => {
    ...
    for (const it of items) {
      ...
      const groupItems = items.filter((x) => (group.itemIds || []).includes(x.id));
      const qOpts = [60, 90, 120, 180].filter((s) => s !== group.restSec);

      rendered.push(
        <div key={group.id} className="rounded-2xl border-2 border-success/40 bg-success/5 p-2 space-y-2">
          <div className="flex items-center justify-between gap-2 px-1">
            <SupersetBadge type={group.type} />
            <div className="flex items-center gap-1.5">
              <span className="text-[10px] text-subtle">{groupItems.length} вправи разом</span>
              <button
                type="button"
          ...
          {!activeWorkout.endedAt && (
            <div className="flex flex-wrap items-center gap-2 px-1 pt-1 border-t border-success/20">
              ...
              <button
                ...
                onClick={() => setRestTimer({ remaining: group.restSec || 60, total: group.restSec || 60 })}
              >
                {group.restSec || 60} с ★
              </button>
              {qOpts.map((sec) => (
  ...
  return (
    <div className="bg-panel border border-line/60 rounded-2xl p-4 shadow-card">
      ...
      <details className="mt-3 rounded-xl border border-line/60 bg-panelHi/50 px-3 py-2">
        ...
        <div className="mt-2 space-y-2">
          <label className="block text-[10px] text-subtle" htmlFor={workoutStartId}>
            Початок
          </label>
          <input
            id={workoutStartId}
          ...
          {activeWorkout.endedAt ? (
            <>
              <label className="block text-[10px] text-subtle" htmlFor={workoutEndId}>
